...
# xw_set / xw_clear 定义于 wallpaper-lib.sh (共享给 rofi 脚本)。
...
set_wallpaper_to_screen() {
    ...
    case "$Type" in
    "video" | "page")
        xw_set "$Type" "$screen_size" "Screen" "$filepath" "${WALLPAPER_ROTATION:-}" ||
            { error "xwallpaper set failed"; return; }
        echo "$filepath|${WALLPAPER_ROTATION:-}" >"$wallpaper_full_latest"
        ;;
    "image")
        xw_set "image" "$screen_size" "Screen" "$filepath" || {
...
set_wallpaper_to_monitor() {
    ...
    local Type=$(detect_file_type "$filepath")
    local rect="${width}x${height}+${x}+${y}"

    clean_target "mon" "$monitor_index"

    case "$Type" in
    "video" | "page")
        xw_set "$Type" "$rect" "$mon_name" "$filepath" "${WALLPAPER_ROTATION:-}" ||
            { error "xwallpaper set failed"; return; }
        echo "$filepath|${WALLPAPER_ROTATION:-}" >"${wallpaper_latest}_${monitor_index}"
        ;;
    "image")
        xw_set "image" "$rect" "$mon_name" "$filepath" || {
            error "xwallpaper set failed"
            return
        }
...
set_wallpaper_to_group() {
    ...
    xw_set "$Type" "${w}x${h}+${x}+${y}" "$target" "$filepath" "${WALLPAPER_ROTATION:-}" ||
        { error "xwallpaper set failed"; return 1; }
